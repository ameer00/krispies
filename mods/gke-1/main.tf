...
resource "google_container_cluster" "gke-0" {
  count                   = "${var.count}"
  name                    = "gke-subnet-${count.index + var.pad}-0"
  zone                    = "${var.zone1}"
  project		              = "${element(var.project, var.svc_count)}"
  network                 = "${var.vpc}"
  subnetwork              = "${element(var.subnet, count.index + var.pad)}"
  initial_node_count      = 31
//  private_cluster         = "true"
//  master_ipv4_cidr_block  = "${cidrsubnet(var.master1_ip_cidr, 9, count.index + var.pad)}"
  ip_allocation_policy    = {
   cluster_secondary_range_name  = "pod-${replace(replace(cidrsubnet(var.pod_ip_cidr, 9, count.index + var.pad), ".", "-"), "/", "-")}"
   services_secondary_range_name = "svc1-${replace(replace(cidrsubnet(var.svc1_ip_cidr, 9, count.index + var.pad), ".", "-"), "/", "-")}"
   }
...
resource "google_container_cluster" "gke-1" {
  count                   = "${var.count}"
  name                    = "gke-subnet-${count.index + var.pad}-1"
  zone                    = "${var.zone2}"
  project		              = "${element(var.project, var.svc_count)}"
  network                 = "${var.vpc}"
  subnetwork              = "${element(var.subnet, count.index + var.pad)}"
  initial_node_count      = 31
//  private_cluster         = "true"
//  master_ipv4_cidr_block  = "${cidrsubnet(var.master2_ip_cidr, 9, count.index + var.pad)}"
  ip_allocation_policy    = {
   cluster_secondary_range_name  = "pod-${replace(replace(cidrsubnet(var.pod_ip_cidr, 9, count.index + var.pad), ".", "-"), "/", "-")}"
   services_secondary_range_name = "svc2-${replace(replace(cidrsubnet(var.svc2_ip_cidr, 9, count.index + var.pad), ".", "-"), "/", "-")}"
   }
//  depends_on = ["google_compute_subnetwork.subnet"]
}
...
resource "google_container_cluster" "gke-2" {
  count                   = "${var.count}"
  name                    = "gke-subnet-${count.index + var.pad}-2"
  zone                    = "${var.zone1}"
  project		              = "${element(var.project, var.svc_count)}"
  network                 = "${var.vpc}"
  subnetwork              = "${element(var.subnet, count.index + var.pad)}"
  initial_node_count      = 31
//  private_cluster         = "true"
//  master_ipv4_cidr_block  = "${cidrsubnet(var.master3_ip_cidr, 9, count.index + var.pad)}"
  ip_allocation_policy    = {
   cluster_secondary_range_name  = "pod-${replace(replace(cidrsubnet(var.pod_ip_cidr, 9, count.index + var.pad), ".", "-"), "/", "-")}"
   services_secondary_range_name = "svc3-${replace(replace(cidrsubnet(var.svc3_ip_cidr, 9, count.index + var.pad), ".", "-"), "/", "-")}"
   }
//  depends_on = ["google_compute_subnetwork.subnet"]
}
...
resource "google_container_cluster" "gke-3" {
  count                   = "${var.count}"
  name                    = "gke-subnet-${count.index + var.pad}-3"
  zone                    = "${var.zone2}"
  project		              = "${element(var.project, var.svc_count)}"
  network                 = "${var.vpc}"
  subnetwork              = "${element(var.subnet, count.index + var.pad)}"
  initial_node_count      = 31
//  private_cluster         = "true"
//  master_ipv4_cidr_block  = "${cidrsubnet(var.master4_ip_cidr, 9, count.index + var.pad)}"
  ip_allocation_policy    = {
   cluster_secondary_range_name  = "pod-${replace(replace(cidrsubnet(var.pod_ip_cidr, 9, count.index + var.pad), ".", "-"), "/", "-")}"
   services_secondary_range_name = "svc4-${replace(replace(cidrsubnet(var.svc4_ip_cidr, 9, count.index + var.pad), ".", "-"), "/", "-")}"
   }
//  depends_on = ["google_compute_subnetwork.subnet"]
}
