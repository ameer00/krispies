...
resource "google_compute_subnetwork" "subnet" {
 ...
 region             = "${var.region}"
 secondary_ip_range = {
      range_name    = "pod-${replace(cidrhost(cidrsubnet(var.pod_ip_cidr, 9, count.index), 2), ".", "-")}"
      ip_cidr_range = "${cidrsubnet(var.pod_ip_cidr, 9, count.index)}"
  }
 secondary_ip_range = {
      range_name    = "svc1-${replace(cidrhost(cidrsubnet(var.svc1_ip_cidr, 9, count.index), 2), ".", "-")}"
      ip_cidr_range = "${cidrsubnet(var.svc1_ip_cidr, 9, count.index)}"
  }
 secondary_ip_range = {
      range_name    = "svc2-${replace(cidrhost(cidrsubnet(var.svc2_ip_cidr, 9, count.index), 2), ".", "-")}"
      ip_cidr_range = "${cidrsubnet(var.svc2_ip_cidr, 9, count.index)}"
  }
 secondary_ip_range = {
      range_name    = "svc3-${replace(cidrhost(cidrsubnet(var.svc3_ip_cidr, 9, count.index), 2), ".", "-")}"
      ip_cidr_range = "${cidrsubnet(var.svc3_ip_cidr, 9, count.index)}"
  }
 secondary_ip_range = {
      range_name    = "svc4-${replace(cidrhost(cidrsubnet(var.svc4_ip_cidr, 9, count.index), 2), ".", "-")}"
...
resource "google_container_cluster" "gke-0" {
  ...
  ip_allocation_policy    = {
   cluster_secondary_range_name  = "pod-${replace(cidrhost(cidrsubnet(var.pod_ip_cidr, 9, count.index), 2), ".", "-")}"
   services_secondary_range_name = "svc1-${replace(cidrhost(cidrsubnet(var.svc1_ip_cidr, 9, count.index), 2), ".", "-")}"
   }
...
resource "google_container_cluster" "gke-1" {
  ...
  ip_allocation_policy    = {
   cluster_secondary_range_name  = "pod-${replace(cidrhost(cidrsubnet(var.pod_ip_cidr, 9, count.index), 2), ".", "-")}"
   services_secondary_range_name = "svc2-${replace(cidrhost(cidrsubnet(var.svc1_ip_cidr, 9, count.index), 2), ".", "-")}"
   }
  depends_on = ["google_compute_subnetwork.subnet"]
}
...
resource "google_container_cluster" "gke-2" {
  ...
  ip_allocation_policy    = {
   cluster_secondary_range_name  = "pod-${replace(cidrhost(cidrsubnet(var.pod_ip_cidr, 9, count.index), 2), ".", "-")}"
   services_secondary_range_name = "svc3-${replace(cidrhost(cidrsubnet(var.svc1_ip_cidr, 9, count.index), 2), ".", "-")}"
   }
  depends_on = ["google_compute_subnetwork.subnet"]
}
...
resource "google_container_cluster" "gke-3" {
  ...
  ip_allocation_policy    = {
   cluster_secondary_range_name  = "pod-${replace(cidrhost(cidrsubnet(var.pod_ip_cidr, 9, count.index), 2), ".", "-")}"
   services_secondary_range_name = "svc4-${replace(cidrhost(cidrsubnet(var.svc1_ip_cidr, 9, count.index), 2), ".", "-")}"
   }
  depends_on = ["google_compute_subnetwork.subnet"]
}
